...
describes what users get. Pick a format with `-o`, or let catfacts choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
catfacts <command> -o table   # aligned columns for reading
catfacts <command> -o jsonl   # one JSON object per line, for piping
catfacts <command> -o json    # a single JSON array
catfacts <command> -o csv     # spreadsheet friendly
catfacts <command> -o tsv     # tab-separated
catfacts <command> -o url     # just the URL column
catfacts <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
catfacts <command> --fields id,title,url
```
...
```bash
catfacts <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
catfacts <command>            # a table, because this is a terminal
catfacts <command> | wc -l    # JSONL, because this is a pipe
```
