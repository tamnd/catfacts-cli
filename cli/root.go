// Package cli assembles the catfacts command tree from the catfacts
...
// NewApp assembles the kit application from the catfacts domain. The
...
//
// To add a command, declare it in catfacts/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := catfacts.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(catfacts.Domain{}).Register(app)
